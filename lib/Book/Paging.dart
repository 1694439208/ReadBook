...
class Paging_algorithm {
  ...
  Paging_algorithm(
      ...
      String data_Text, this.width, this.height, this.style, this.Index,this.context,
    ...
    //替换规则
    String washpest = r"(PS|ps)(.)*(|\\n)";
    //将小说内容中的PS全部替换为“”
    var data = data_Text.replaceAll(RegExp(washpest), "");
    ChapterList = data.split(RegExp(pest)); //分章

    //计算当前阅读位置的章节
    if (ChapterIndex < ChapterList.length) {
      var ad = ChapterTextPainter.CreatePagerData(
          ArgData(ChapterList[ChapterIndex], width, height, style, lineHeight,context));
    ...
    var list = RegExp(pest).allMatches(data);
    Titlenum.add("开始");
  ...
  void CreateChapIndex(
    ...
    if (!ChapterPageList.containsKey(index)) {
      ...
      if (index < ChapterList.length) {
        //没有到最后一章
        var ad = ChapterTextPainter.CreatePagerData(
            ArgData(ChapterList[index], width1, height1, style1, lineHeight,context));
